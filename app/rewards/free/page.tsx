...
export default function FreeSpinsPage() {
  ...
  useEffect(() => {
    if (!session || !selectedCategory || !selectedTier) return
    void loadAvailablePrizes()
  }, [session, month, selectedCategory, selectedTier])
  ...
  async function loadAvailablePrizes() {
    ...
    setAvailablePrizes((data ?? []).map((row: AvailablePrize) => ({
      ...row,
  ...
  const selectedCategoryName = useMemo(
    () => categories.find((item) => item.slug === selectedCategory)?.name ?? 'Reward',
    [categories, selectedCategory],
  )

  const selectedTierName = useMemo(
    () => tiers.find((tier) => tier.slug === selectedTier)?.name ?? 'Tier',
    [tiers, selectedTier],
  )

  const staffNameById = useMemo(
    () => new Map(staff.map((member) => [member.user_id, member.display_name])),
    [staff],
  )
  ...
  async function spinFreeReward() {
    ...
    if (error || !data?.length) {
      setSpinning(false)
      showMessage(error?.message ?? 'The free spin could not be recorded.', 'error')
      return
    }
    ...
    window.setTimeout(async () => {
      ...
      await loadWins(selectedChild.id)
      showMessage('Free spin recorded. Monthly earned spins were not changed.', 'success')
    }, 4200)
  }

  if (loading && !session) {
    return <main className="login-wrap"><div className="card login-card">Loading Free Spins…</div></main>
  }
    ...
    return (
      <main className="login-wrap">
        <section className="card login-card">
          <h1>Free Spins</h1>
          <p className="subtle">Sign in through Juanita Hub before awarding a free reward spin.</p>
  ...
  if (!profile?.active) {
    return (
      <main className="login-wrap">
        <section className="card login-card">
          <h1>Free Spins</h1>
          <div className="notice">Your staff account must be active before free spins are available.</div>
          <Link className="ghost" style={{ display: 'inline-block', textDecoration: 'none' }} href="/">Back to Juanita Hub</Link>
        </section>
      </main>
    )
  }
  ...
  return (
    <div className="shell">
      <header className="topbar">
        <div className="brand">Juanita Hub<small>Free Reward Spins</small></div>
      ...
      <main className="main">
        <div className="hero">
          <div>
            <h1>Free Spins</h1>
            <p className="subtle">Award a bonus spin without using the child’s earned monthly spins. Inventory and reward history are still updated automatically.</p>
          </div>
          <label className="field" style={{ minWidth: 180, marginBottom: 0 }}>
            <span style={{ fontWeight: 650 }}>Inventory month</span>
        ...
        <div className="notice" style={{ marginBottom: 16 }}>
          <strong>Free spins do not reduce monthly spins.</strong> Selecting Premium includes Normal + Premium prizes; selecting Special includes Normal + Premium + Special prizes.
        </div>
        ...
        <section style={{ display: 'grid', gridTemplateColumns: 'minmax(280px, 340px) minmax(0, 1fr)', gap: 16, alignItems: 'start' }}>
          <aside className="card">
            ...
            <div className="field">
              ...
              <select value={selectedChildId ?? ''} onChange={(event) => { setSelectedChildId(Number(event.target.value)); setResult(null) }}>
                {children.map((child) => <option key={child.id} value={child.id}>{childName(child)}</option>)}
              </select>
            </div>

            <div className="field">
              <label>Allowed tier</label>
              <select value={selectedTier} onChange={(event) => { setSelectedTier(event.target.value); setResult(null) }}>
                {tiers.map((tier) => <option key={tier.id} value={tier.slug}>{tier.name}</option>)}
              </select>
            </div>

            <div className="notice" style={{ textAlign: 'center', marginTop: 0 }}>
              <strong>{selectedTierName} wheel selected</strong>
            </div>
            ...
            <div className="grid" style={{ gridTemplateColumns: 'repeat(3, 1fr)', gap: 8, marginBottom: 14 }}>
              {categories.map((item) => (
                <button
                  key={item.id}
                  className={selectedCategory === item.slug ? 'primary' : 'ghost'}
                  style={{ padding: '10px 5px' }}
                  onClick={() => { setSelectedCategory(item.slug); setResult(null) }}
                >
                  {categoryIcon(item.slug)}<br />{item.name}
                </button>
              ))}
            </div>

            <div className="field" style={{ marginBottom: 0 }}>
              <label>Reason <span className="subtle">(optional)</span></label>
              <input
                value={reason}
                onChange={(event) => setReason(event.target.value)}
                placeholder="Example: bonus activity"
                maxLength={200}
              />
            </div>
          </aside>
          ...
          <section className="card">
            <div className="section-heading">
              <div>
                <h2>{selectedCategoryName} — {selectedTierName}</h2>
                <p className="subtle">The wheel uses the same weights and {monthLabel(month)} inventory as the monthly Reward Center.</p>
              </div>
            ...
            <div style={{ position: 'relative', width: 'min(500px, 92%)', aspectRatio: '1', margin: '22px auto' }}>
              ...
              <div style={{
                ...
                height: '100%',
                borderRadius: '50%',
                background: weightedGradient,
                border: '8px solid white',
                boxShadow: '0 4px 18px rgba(16,24,40,.16)',
                transform: `rotate(${rotation}deg)`,
                transition: 'transform 4s cubic-bezier(.12,.72,.15,1)',
                display: 'grid',
                placeItems: 'center',
              }}>
                <div style={{ width: 86, height: 86, borderRadius: '50%', background: '#111827', color: 'white', display: 'grid', placeItems: 'center', border: '5px solid white', fontWeight: 800, textAlign: 'center', fontSize: 13 }}>FREE<br />SPIN</div>
              </div>
            </div>

            <div className="grid" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(155px, 1fr))', gap: 8, marginBottom: 14 }}>
              {availablePrizes.map((prize, index) => (
                <div key={prize.prize_id} style={{ display: 'flex', gap: 8, alignItems: 'center', border: '1px solid #e4e7ec', borderRadius: 9, padding: 8 }}>
                  <span style={{ width: 13, height: 13, borderRadius: 99, background: wheelColors[index % wheelColors.length], flex: '0 0 auto' }} />
                  <span>
                    <strong>{prize.prize_name}</strong><br />
                    <span className="subtle" style={{ fontSize: 12 }}>{prize.required_tier_name} • stock {prize.quantity_remaining} • weight {prize.weight}</span>
                  </span>
                </div>
              ))}
            </div>
            ...
            {result && !spinning && (
              <div className="notice success" style={{ textAlign: 'center', fontSize: 18 }}>
                🎉 <strong>{selectedChild ? childName(selectedChild) : 'Child'} won {result.prize_name}!</strong>
              </div>
            )}
            ...
            <button
              ...
              disabled={spinning || !selectedChild || availablePrizes.length === 0}
              onClick={spinFreeReward}
            >
              {spinning ? 'Selecting free reward…' : `FREE SPIN • ${selectedTierName}`}
            </button>

            {availablePrizes.length === 0 && (
              <div className="notice">No eligible in-stock {selectedCategoryName.toLowerCase()} prizes are configured for the {selectedTierName} wheel in {monthLabel(month)}.</div>
            )}
          </section>
        </section>
        ...
        <section className="card" style={{ marginTop: 16 }}>
          <div className="section-heading">
            <div>
              <h2>{selectedChild ? childName(selectedChild) : 'Child'} free-spin history</h2>
              <p className="subtle">These records are separate from monthly earned spins but use the same prize inventory.</p>
            </div>
            <span className="badge">{wins.length} free {wins.length === 1 ? 'spin' : 'spins'}</span>
          </div>

          {wins.length === 0 ? (
            <div className="empty">No free spins recorded for this child in {monthLabel(month)}.</div>
          ) : wins.map((win) => (
            <div className="summary-row" key={win.id}>
              <span>
                <strong>Free Spin: {win.prize_name_snapshot}</strong><br />
              ...
              <span className="subtle" style={{ textAlign: 'right' }}>
                {new Date(win.won_at).toLocaleString()}<br />
                {staffNameById.get(win.recorded_by) ?? 'Staff'}
              </span>
            </div>
          ))}
        </section>
