...
export default function RewardsPage() {
  ...
  async function loadRewardData() {
    ...
    setInventoryEdits(Object.fromEntries(nextInventory.map((row) => [row.prize_id, {
  ...
  async function addPrize() {
    if (profile?.role !== 'admin') {
      showMessage('Admin access is required to add prizes.', 'error')
      return
    }
    if (!newPrizeName.trim()) {
      showMessage('Enter a prize name first.', 'error')
      return
    }
    if (!newPrizeCategory || !newPrizeTier) {
      showMessage('Choose a category and minimum tier.', 'error')
      return
    }
    ...
    <div className="shell">
      <header className="topbar">
        ...
        <div className="toolbar">
          ...
          <Link className="ghost" style={{ textDecoration: 'none' }} href="/">Dashboard</Link>
        </div>
      </header>
      ...
      <main className="main">
        ...
        <section style={{ display: 'grid', gridTemplateColumns: 'minmax(270px, 320px) minmax(0, 1fr)', gap: 16, alignItems: 'start' }}>
          <aside className="card">
            ...
            <div className="field">
              ...
              >
                {roster.map((child) => (
                  <option key={child.child_id} value={child.child_id}>{child.child_name} — {child.remaining_spins} remaining</option>
                ))}
              </select>
            </div>
            ...
            {selectedChild && (
              <div className="grid" style={{ gridTemplateColumns: 'repeat(3, 1fr)', gap: 8, marginBottom: 14 }}>
                {[
                  ['Earned', selectedChild.earned_spins],
                  ['Used', selectedChild.used_spins],
                  ['Remaining', selectedChild.remaining_spins],
                ].map(([label, value]) => (
            ...
            <div className="grid" style={{ gridTemplateColumns: 'repeat(3, 1fr)', gap: 8 }}>
              {categories.map((item) => (
                ...
                >
                  {item.slug === 'candy' ? '🍬' : item.slug === 'toys' ? '🧸' : '🍕'}<br />{item.name}
                </button>
              ))}
            </div>
          </aside>
            ...
            <div className="grid" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: 8, marginBottom: 14 }}>
              {availablePrizes.map((prize, index) => (
                <div key={prize.id} style={{ display: 'flex', gap: 8, alignItems: 'center', border: '1px solid #e4e7ec', borderRadius: 9, padding: 8 }}>
                  ...
                  <span><strong>{prize.name}</strong><br /><span className="subtle" style={{ fontSize: 12 }}>Weight {prize.weight}</span></span>
                </div>
              ))}
            </div>
        ...
        {profile.role === 'admin' && (
          <details open id="reward-setup" className="card" style={{ marginTop: 16 }}>
            ...
            <section style={{ border: '1px solid #e4e7ec', borderRadius: 14, padding: 16, marginBottom: 16, background: '#f8fafc' }}>
              ...
              <div className="grid" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: 10 }}>
                <div className="field" style={{ minWidth: 210 }}>
                  ...
                  <input value={newPrizeName} onChange={(event) => setNewPrizeName(event.target.value)} placeholder="Example: Airheads" />
                </div>
                <div className="field">
                  <label>Category</label>
                  <select value={newPrizeCategory ?? ''} onChange={(event) => setNewPrizeCategory(Number(event.target.value))}>
                    {categories.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}
                  </select>
                </div>
                <div className="field">
                  <label>Minimum tier</label>
                  <select value={newPrizeTier ?? ''} onChange={(event) => setNewPrizeTier(Number(event.target.value))}>
                    {tiers.map((tier) => <option key={tier.id} value={tier.id}>{tier.name} ({tier.min_spins}+ spins)</option>)}
                  </select>
                </div>
                <div className="field">
                  <label>Stock</label>
                  <input type="number" min="0" step="1" value={newPrizeStock} onChange={(event) => setNewPrizeStock(Number(event.target.value))} />
                </div>
                <div className="field">
                  <label>Weight</label>
                  <input type="number" min="0.01" step="0.25" value={newPrizeWeight} onChange={(event) => setNewPrizeWeight(Number(event.target.value))} />
                </div>
              </div>
            ...
            {prizes.length === 0 && <div className="notice">No prizes have been added yet. Use the form above to create the first Candy, Toys, or Food item.</div>}
            ...
            {categories.map((item) => (
              <section key={item.id} style={{ marginTop: 20 }}>
                ...
                {prizes.filter((prize) => prize.category_id === item.id).map((prize) => {
                  ...
                  const edit = inventoryEdits[prize.id] ?? {
                    remaining: Number(row?.quantity_remaining ?? 0),
                    ...
                    <div className="summary-row" key={prize.id} style={{ alignItems: 'end', flexWrap: 'wrap' }}>
                      ...
                      <span className="toolbar">
                        <label className="field" style={{ margin: 0, width: 90 }}>
                          <span>Remaining</span>
                          <input type="number" min="0" step="1" value={edit.remaining} onChange={(event) => updateInventoryEdit(prize.id, { remaining: Number(event.target.value) })} />
                        </label>
                        <label className="field" style={{ margin: 0, width: 85 }}>
                          <span>Weight</span>
                          <input type="number" min="0.01" step="0.25" value={edit.weight} onChange={(event) => updateInventoryEdit(prize.id, { weight: Number(event.target.value) })} />
                        </label>
                        <label style={{ display: 'flex', gap: 6, alignItems: 'center', paddingBottom: 10 }}>
                          <input type="checkbox" checked={edit.enabled} onChange={(event) => updateInventoryEdit(prize.id, { enabled: event.target.checked })} /> Enabled
                        </label>
                        <button className="ghost" disabled={savingPrizeId === prize.id} onClick={() => saveInventory(prize.id)}>
