...
type Program = {
  ...
  registration_mode: 'full' | 'renewal' | 'summer_short' | 'permission_only' | 'short_youth' | 'adult_short'
...
const blankProgram: ProgramForm = {
  ...
  program_type: 'club',
  audience: 'youth',
  registration_mode: 'permission_only',
  season_label: '',
  description: '',
  location: '',
  starts_on: '',
...
const registrationLabels: Record<Program['registration_mode'], string> = {
...
export default function ProgramsPage() {
  ...
  async function loadData() {
    ...
    setLoading(true)
    setMessage('')
    const [profileResult, programsResult, enrollmentsResult, childrenResult, householdsResult, linksResult, adultsResult] = await Promise.all([
    ...
    setMessage(profileResult.error?.message || programsResult.error?.message || enrollmentsResult.error?.message || childrenResult.error?.message || householdsResult.error?.message || linksResult.error?.message || adultsResult.error?.message || '')
    setLoading(false)
  }
  ...
  async function createProgram() {
    if (profile?.role !== 'admin' || !programForm.name.trim() || saving) return
    setSaving(true)
    setMessage('')
    ...
    const { data, error } = await supabase.from('programs').insert({
      ...
      registration_mode: programForm.registration_mode,
    ...
    if (error) setMessage(error.message)
      ...
      setMessage('Program created. You can now build its roster.')
      await loadData()
    }
    setSaving(false)
  }
    ...
    <div className="shell">
      ...
      <main className="main programs-page">
        <section className="hero programs-hero">
          <div><span className="programs-eyebrow">Programs</span><h1>Programs & Enrollments</h1><p className="subtle">Use one enrollment system for afterschool, summer, clubs, workshops, special programs, and adult classes.</p></div>
          <div className="programs-mode-pill">Flexible registration modes</div>
        </section>
        ...
        {profile.role === 'admin' && (
          <section className="card program-builder">
            <div className="program-builder-heading"><div><small>Admin setup</small><h2>Create a program</h2></div><span>Choose the form style now; the parent-facing form comes later.</span></div>
            <div className="program-form-grid">
              <label className="field full"><span>Program name</span><input value={programForm.name} onChange={(event) => setProgramForm((current) => ({ ...current, name: event.target.value }))} placeholder="Robotics Club, Summer 2027, Adult Computer Basics…" /></label>
              <label className="field"><span>Program type</span><select value={programForm.program_type} onChange={(event) => setProgramForm((current) => ({ ...current, program_type: event.target.value as Program['program_type'] }))}>{Object.entries(typeLabels).map(([value, label]) => <option value={value} key={value}>{label}</option>)}</select></label>
              <label className="field"><span>Audience</span><select value={programForm.audience} onChange={(event) => setProgramForm((current) => ({ ...current, audience: event.target.value as Program['audience'] }))}><option value="youth">Youth</option><option value="adult">Adult</option><option value="family">Family</option><option value="mixed">Mixed</option></select></label>
              <label className="field"><span>Registration style</span><select value={programForm.registration_mode} onChange={(event) => setProgramForm((current) => ({ ...current, registration_mode: event.target.value as Program['registration_mode'] }))}>{Object.entries(registrationLabels).map(([value, label]) => <option value={value} key={value}>{label}</option>)}</select></label>
            ...
            <div className="program-builder-actions"><button type="button" className="primary" onClick={() => void createProgram()} disabled={saving || !programForm.name.trim()}>{saving ? 'Saving…' : 'Create program'}</button></div>
          </section>
        )}
        ...
        <div className="programs-layout">
          <section className="card programs-list-panel">
            ...
            <div className="programs-list">
              ...
              {programs.length === 0 && <div className="programs-empty">No programs yet. The first one can be a club, summer program, class, or any other center activity.</div>}
            </div>
          </section>

          <section className="card program-detail-panel">
            {!selected ? (
              <div className="programs-empty large"><strong>Select a program</strong><span>Its registration style and roster will appear here.</span></div>
            ) : (
              <>
                ...
                <div className="program-facts">
                  <div><small>Registration</small><strong>{registrationLabels[selected.registration_mode]}</strong></div>
