    ...
    <main className="main registration-admin-page">
      ...
      {tab === 'setup' ? <div className="registration-admin-grid">
        ...
        <section className="card registration-setup-panel">{!selectedProgram?<div className="registration-empty"><strong>Select a program</strong><span>Its public registration settings will appear here.</span></div>:<>
          ...
          <div className="registration-settings-grid"><label className="field full"><span>Welcome / instructions</span><textarea rows={4} value={intro} onChange={e=>setIntro(e.target.value)} placeholder="Tell families what to know before they begin…" /></label><label className="field"><span>Registration opens</span><input type="datetime-local" value={opensAt} onChange={e=>setOpensAt(e.target.value)} /></label><label className="field"><span>Registration closes</span><input type="datetime-local" value={closesAt} onChange={e=>setClosesAt(e.target.value)} /></label></div><div className="registration-actions"><button className="primary" onClick={()=>void savePortalSettings()} disabled={saving}>Save portal settings</button></div>
        ...
        <section className="card registration-review-panel">{!selectedSubmission?<div className="registration-empty"><strong>Select a submission</strong><span>Review its information before approving it.</span></div>:<><div className="registration-review-heading"><div><small>Submission {selectedSubmission.submission_code}</small><h2>{selectedSubmission.submitter_name || 'Registration submission'}</h2><p>{programById.get(selectedSubmission.program_id)?.name} • {modeLabels[selectedSubmission.registration_mode]}</p></div><span className={`registration-status ${selectedSubmission.status}`}>{selectedSubmission.status.replace('_',' ')}</span></div>
          ...
          {Array.isArray(payload.consents)&&payload.consents.length>0&&<section className="registration-review-section"><h3>Permissions</h3>{payload.consents.map((c:any)=><p key={c.id}><strong>{c.agreed?'✓':'—'} {c.title}</strong> {c.required?'(required)':''}</p>)}</section>}
