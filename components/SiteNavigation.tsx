...
const navGroups: NavGroup[] = [
  {
    ...
    items: [
      ...
      { href: '/card-tracking', label: 'Card Tracking', description: 'Daily behavior cards, statuses, summaries, and child history.' },
...
export default function SiteNavigation() {
  ...
  useEffect(() => {
    ...
    async function loadAccess() {
      ...
      const { data } = await supabase.from('staff_profiles').select('active, role').eq('user_id', userId).maybeSingle()
      if (mounted) setAccess({ active: Boolean(data?.active), role: data?.role === 'admin' || data?.role === 'staff' ? data.role : null })
    }
    void loadAccess()
  ...
  function closeMenus() { setOpenGroup(null); setMobileOpen(false) }
  useEffect(() => { closeMenus() }, [pathname])
  useEffect(() => {
    function handlePointerDown(event: PointerEvent) { if (navRef.current && !navRef.current.contains(event.target as Node)) setOpenGroup(null) }
    function handleKeyDown(event: KeyboardEvent) { if (event.key === 'Escape') { setOpenGroup(null); setMobileOpen(false) } }
  ...
  if (pathname.startsWith('/kiosk') || pathname.startsWith('/register') || pathname.startsWith('/reset-password') || pathname.startsWith('/forgot-password')) return null
  ...
  function visibleItems(group: NavGroup) { return group.items.filter((item) => !item.adminOnly || access.role === 'admin') }
  function currentItemHref(items: NavItem[]) {
    return items.filter((item) => item.href && routeMatches(item.href)).map((item) => item.href as string).sort((a, b) => b.length - a.length)[0] ?? null
  }
  function groupIsCurrent(group: NavGroup) { return currentItemHref(group.items) !== null }
  function toggleGroup(label: string) { setOpenGroup((current) => current === label ? null : label) }
      ...
      <nav ref={navRef} className="site-nav" aria-label="Juanita Hub site navigation">
        <div className="site-nav-inner">
          <Link className="site-nav-brand" href="/" aria-label="Juanita Hub dashboard" onClick={closeMenus}>
            ...
            <span><strong>Juanita Hub</strong><small>Community Center Operations</small></span>
          </Link>
          <button className="site-nav-toggle" type="button" aria-expanded={mobileOpen} aria-controls="juanita-site-menu" onClick={() => { setOpenGroup(null); setMobileOpen((open) => !open) }}>
            <span className="site-nav-toggle-lines" aria-hidden="true"><span /><span /><span /></span><span>{mobileOpen ? 'Close' : 'Menu'}</span>
          </button>
          <div id="juanita-site-menu" className={`site-nav-menu ${mobileOpen ? 'open' : ''}`}>
            <Link href="/" className={`site-nav-link ${pathname === '/' ? 'active' : ''}`} aria-current={pathname === '/' ? 'page' : undefined} onClick={closeMenus}>Dashboard</Link>
            {navGroups.map((group) => {
              ...
              return (
                <div className={`site-nav-group ${active ? 'active' : ''} ${isOpen ? 'open' : ''}`} key={group.label}>
                  <button type="button" className="site-nav-group-trigger" aria-expanded={isOpen} aria-controls={groupId} onClick={() => toggleGroup(group.label)}><span>{group.label}</span><span className="site-nav-chevron" aria-hidden="true">⌄</span></button>
                  {isOpen && <div className="site-nav-dropdown" id={groupId}>{items.map((item) => {
                    if (item.comingSoon || !item.href) return <div className="site-nav-dropdown-item coming-soon" key={item.label}><span><strong>{item.label}</strong>{item.description && <small>{item.description}</small>}</span><span className="site-nav-soon">Coming soon</span></div>
                    const current = activeHref === item.href
                    return <Link className={`site-nav-dropdown-item ${current ? 'active' : ''}`} href={item.href} key={item.href} aria-current={current ? 'page' : undefined} onClick={closeMenus}><span><strong>{item.label}</strong>{item.description && <small>{item.description}</small>}</span></Link>
                  })}</div>}
                </div>
              )
            })}
